<template>
  <div
    v-bind="rootAttrs"
    class="pu-chips-editor"
    :class="rootClass"
    @click="handleRootClick"
  >
    <div v-if="$slots.prefix" class="pu-chips-editor__prefix">
      <slot name="prefix" />
    </div>

    <PuChipGroup class="pu-chips-editor__content" gap="xs">
      ...
      <input
        ref="inputRef"
        v-bind="inputAttrs"
        class="pu-chips-editor__control"
        type="text"
        :value="draftText"
    ...
    <div v-if="showClear || $slots.suffix" class="pu-chips-editor__suffix">
      <button
        v-if="showClear"
        type="button"
        class="i-mdi-close-circle pu-chips-editor__clear"
        :disabled="props.disabled"
...
import {
  puChipsEditorEmits,
  puChipsEditorProps,
  type PuChipsEditorChangeContext,
  type PuChipsEditorChangeSource,
  type PuChipsEditorValue,
} from "./puChipsEditor";

const props = defineProps(puChipsEditorProps);
const emit = defineEmits(puChipsEditorEmits);
...
const rootClass = computed(() => [
  `pu-chips-editor--size-${props.size}`,
  `pu-chips-editor--variant-${props.variant}`,
  `pu-chips-editor--tone-${props.tone}`,
  `pu-chips-editor--shape-${props.shape}`,
  {
...
function emitValue(nextValue: PuChipsEditorValue, event: Event): void {
...
function addValues(
  ...
  const context: PuChipsEditorChangeContext = { event, source };
...
function commitDraft(event: Event, source: PuChipsEditorChangeSource): boolean {
...
<style lang="scss" scoped src="./puChipsEditor.scss"></style>
